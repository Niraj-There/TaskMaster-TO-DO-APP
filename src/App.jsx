...
// Task List Component
const TaskList = ({ tasks, onToggleTask, onEditTask, onDeleteTask }) => (
    ...
    <div className={`relative bg-[#F0F5F9] bg-opacity-90 backdrop-blur-md rounded-xl shadow-lg p-5 border transition-all duration-300 ease-in-out transform hover:scale-[1.01] ${task.completed ? 'opacity-70 border-gray-200 dark:border-gray-800' : 'border-[#1E2022] dark:border-[#F0F5F9]'}`}>
      ...
      <div className="flex items-start">
        ...
        <div className="flex-grow">
          <h3 className={`text-xl font-semibold font-montserrat ${task.completed ? 'line-through text-gray-500 dark:text-gray-400' : 'text-[#1E2022] dark:text-[#F0F5F9]'}`}>
            {task.title}
          </h3>
          {task.description && (
            <p className="text-[#1E2022] text-sm mt-1 font-montserrat dark:text-[#F0F5F9]">{task.description}</p>
          )}
          <div className="flex flex-wrap items-center text-xs text-[#1E2022] mt-2 space-x-3 font-montserrat dark:text-[#F0F5F9]">
            {dueDate && (
          ...
          {task.subtasks && task.subtasks.length > 0 && ( // Display subtasks
            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-800">
              <p className="text-[#1E2022] text-sm font-semibold mb-2 dark:text-[#F0F5F9]">Subtasks:</p>
              <ul className="space-y-1">
                {task.subtasks.map(subtask => (
                  <li key={subtask.id} className={`flex items-center text-sm ${subtask.completed ? 'line-through text-gray-500 dark:text-gray-400' : 'text-[#1E2022] dark:text-[#F0F5F9]'}`}>
                    <i className={`mr-2 ${subtask.completed ? 'bi bi-check-square-fill text-[#1E2022] dark:text-[#F0F5F9]' : 'bi bi-square text-[#1E2022] dark:text-[#F0F5F9]'}`}></i>
                    {subtask.text}
                  </li>
                ))}
              </ul>
        ...
        <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 ml-4">
          <button
            onClick={() => onEdit(task)}
            className="p-2 rounded-full text-[#1E2022] hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[#1E2022] transition-colors duration-200 dark:text-[#F0F5F9] dark:hover:bg-gray-800 dark:focus:ring-[#F0F5F9]"
            title="Edit Task"
          >
          ...
          <button
            ...
            className="p-2 rounded-full text-[#1E2022] hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-red-500 transition-colors duration-200 dark:text-[#F0F5F9] dark:hover:bg-gray-800 dark:focus:ring-red-500"
            title="Delete Task"
          >
